...
const currentUser = $('[name="gbs-visitor"]').attr('content');
const currentHost = $('[name="gbs-url"]').attr('content');
const apiHost = $('[name="gbs-api-url"]').attr('content');
const headers = { 'Authorization': 'Bearer ' + $('[name="gbs-ref"]').attr('content'), 'Accept': $('.mime-type').val(), 'X-localization': navigator.language };
// Modals
...
$(function () {
    ...
    jQuery('#outflow_date, [id^="outflow_date"], [id^="end_date"]').datetimepicker({
        format: 'd/m/Y H:i'
    });
